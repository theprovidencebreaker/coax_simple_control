...
bool CoaxSimpleControl::setWaypoint(coax_simple_control::SetWaypoint::Request &req1, coax_simple_control::SetWaypoint::Response &out) {
  out.result = 0;
...
void CoaxSimpleControl::stabilizationControl(void) {
  ...
  double yaw_deg, yaw;
  double mag_conv1, mag_conv2;
  double K_xy[2][8], K_hy[2][4];
  double hz_intern, delta_t, delta_t2;
  double lqr_states[8], lqr_des[8], lqr_error[8];
  double cont_xy[2], u_des, v_des;
  double xgyro_smooth, ygyro_smooth;
  double altitude_v, e_altitude_v;
  ...
  hz_intern = 100.000;
  delta_t = 1.00/hz_intern;
  delta_t2 = 0.07;
  
 
  // Vx_global = (coax_global_x - x_vel_hist_1)/delta_t2;
  // Vy_global = (coax_global_y - y_vel_hist_1)/delta_t2;

  acc_xc = 0.4*accel_x + 0.2*acc_hx[0] + 0.2*acc_hx[1] + 0.2*acc_hx[2];
  ...
  acc_yc = 0.4*accel_y + 0.2*acc_hy[0] + 0.2*acc_hy[1] + 0.2*acc_hy[2];
  ...
  vx_facc = acc_xc*delta_t2 + vx_facc_old;
  vy_facc = acc_xc*delta_t2 + vy_facc_old;
   

  
  //if (mil_count % 7 == 0 ) {
      
  if(coax_global_x != Gx_old[0]) {     

    Vx_global = 0.7*(coax_global_x - Gx_old[0])/delta_t2 + 0.2*(Gx_old[0] - Gx_old[1])/delta_t2 + 0.1*(Gx_old[1] - Gx_old[2])/delta_t2;
    Vy_global = 0.7*(coax_global_y - Gy_old[0])/delta_t2 + 0.2*(Gy_old[0] - Gy_old[1])/delta_t2 + 0.1*(Gy_old[1] - Gy_old[2])/delta_t2;

    // Vx_global = (coax_global_x - Gx_old[0])/delta_t2; 
    // Vy_global = (coax_global_y - Gy_old[0])/delta_t2;
    ...
    Gx_old[2] = Gx_old[1];
    Gx_old[1] = Gx_old[0];
    Gx_old[0] = coax_global_x;

    Gy_old[2] = Gy_old[1];
    Gy_old[1] = Gy_old[0];
    Gy_old[0] = coax_global_y;
  ...
  /*
  if (mil_count % 50 == 0 ) {
    ROS_INFO("x y error: %f, %f ", e_x, e_y);
    ROS_INFO("Gx and Gy des: %f, %f", Gx_des, Gy_des);
  }
  */
  ...
  cont_xy[0] = K_xy[0][0]*lqr_error[0] + K_xy[0][1]*lqr_error[1] + K_xy[0][2]*lqr_error[2] + K_xy[0][3]*lqr_error[3] + K_xy[0][4]*lqr_error[4] + K_xy[0][5]*lqr_error[5] + K_xy[0][6]*lqr_error[6] + K_xy[0][7]*lqr_error[7];
  cont_xy[1] = K_xy[1][0]*lqr_error[0] + K_xy[1][1]*lqr_error[1] + K_xy[1][2]*lqr_error[2] + K_xy[1][3]*lqr_error[3] + K_xy[1][4]*lqr_error[4] + K_xy[1][5]*lqr_error[5] + K_xy[1][6]*lqr_error[6] + K_xy[1][7]*lqr_error[7];
  ...
  */
  ...
  if (mil_count % 50 == 0 ) {
    ...
    ROS_INFO("altitude error : %f", e_altitude);
  }
  */
